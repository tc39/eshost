...
export class ConsoleAgent extends Agent {
  static RUNTIME = `
...
`;
  ...
  constructor(options) {
    ...
    Object.defineProperties(this, {
      ...
      [TEMP_PATH]: {
        value: fs.mkdtempSync(path.join(os.tmpdir(), "eshost-")),
        writable: true,
        enumerable: false,
      },
    });
  ...
  async evalScript(code, options = {}) {
    ...
    if (typeof code === "object" && code.contents) {
      ...
      if (options.module || attrs.flags.module || hasModuleSpecifier(contents)) {
    ...
    if (hasDependencies) {
      ...
      dependencies.forEach((file) => {
        ...
        if (absname !== tempfile) {
          sources.push([absname, depsource]);
        }
      });
    }
    ...
    this[CHILD_PROCESS].stdout.on("data", (str) => {
      stdout += str;
    });
    this[CHILD_PROCESS].stderr.on("data", (str) => {
      stderr += str;
    });
  ...
  compile(code, options) {
      ...
      recast.visit(ast, {
        visitNode(node) {
          ...
          if (hostOptions.shortName) {
            ...
            if (node.value.type === "Identifier" && node.value.name === "$262") {
    ...
    if (!runtime) {
      ...
      const header = prologue ? prologue[0] + runtime : runtime;

      const body = prologue ? code.slice(prologue[0].length) : code;
