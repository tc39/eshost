...
const runtimeHtml = fs.readFileSync(new URL("../runtimes/browser.html", import.meta.url), "utf8");
const runtimeJs = fs.readFileSync(new URL("../runtimes/browser.js", import.meta.url), "utf8");
const stackParserPath = Path.join(
  Path.dirname(require.resolve("error-stack-parser")),
...
function httpHandler(req, res) {
  ...
  switch (url.pathname) {
    ...
    case "/runtime.js":
      sendRuntime(req, res);
      break;
    case "/error-stack-parser.js":
      sendStackParser(req, res);
      break;
    default:
      res.writeHead(404);
      res.end();
...
export function clientIdStopped(id) {
...
export function waitForClientId(id) {
  let handler = SocketHandler.byId.get(id);
...
export function waitForResult(id) {
  let handler = SocketHandler.byId.get(id);
  return new Promise((res) => {
