...
class GraalJSAgent extends ConsoleAgent {
  static RUNTIME = fs.readFileSync(runtimePath.for("graaljs"), "utf8");
  ...
  parseError(str) {
    ...
    const match = hasStack ? ematch : cmatch;
    ...
    let stack;
    if (hasStack) {
      stack = parseStack(stackStr.trim());
    } else {
      stack = [];
    }
