...
class HermesAgent extends ConsoleAgent {
  static RUNTIME = fs.readFileSync(runtimePath.for("hermes"), "utf8");
  ...
  async evalScript(code, options = {}) {
    ...
    try {
      ...
      const [, releaseVersion] = /Hermes release version: (.*)/.exec(stdout);
      const [, bytecodeVersion] = /HBC bytecode version: (.*)/.exec(stdout);
