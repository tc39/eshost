...
class LibJSAgent extends ConsoleAgent {
  static RUNTIME = fs.readFileSync(runtimePath.for("libjs"), "utf8");
  ...
  parseError(str) {
    ...
    try {
      ...
      if (typeof parsedMessage["message"] === "string") message = parsedMessage["message"];
    } catch {
      // ignored
    }
    ...
    if (match[3]) {
      ...
      stackTrace.forEach((entry) => {
        ...
        if (trimmedEntry.endsWith(" more calls")) {
          // collapsed 5+ repeated entries
          const calls = parseInt(trimmedEntry.substring(0, trimmedEntry.indexOf(" more calls")));
          for (let i = 0; i < calls; ++i) stack.push(stack[stack.length - 1]);
        } else {
          stack.push({
  ...
  normalizeResult(result) {
    ...
    if (match) {
      ...
      if (result.stdout.endsWith(expectedHintMarker))
        // syntax error source location hint marker
