  ...
  describe(`${type} (${options.hostPath || type})`, function() {
    ...
    describe('normal script evaluation', function() {
      ...
      it('runs thrown custom Errors', async () => {
        const result = await agent.evalScript('function Foo1Error(msg) { this.name = "Foo1Error"; this.message = msg }; Foo1Error.prototype = Error.prototype; throw new Foo1Error("Custom Message");');
        assert.strictEqual(result.stdout, '', 'stdout not present');
      ...
      it('runs thrown errors from eval', async () => {
        const result = await agent.evalScript('eval("\'\\u000Astr\\u000Aing\\u000A\'") === "\\u000Astr\\u000Aing\\u000A"');
        assert.strictEqual(result.stdout, '', 'stdout not present');
      ...
      it('returns errors from evaling in new script', async () => {
        ...
        assert((result.stdout || result.stderr).match(/SyntaxError/m), `Unexpected stdout: ${result.stdout}${result.stderr}`);
      });
      ...
      it('creates "optional" environments correctly (hostArgs)', async () => {
        ...
        const agent = await eshost.createAgent(type, { hostArguments, ...options });
        ...
        return Promise.all(
          records.map(async record => {
            ...
            if (negative) {
              ...
              assert.strictEqual(stdout, expectedStdout, `stdout is "${expectedStdout}"`);
            }
          })
        );
      });
    ...
    describe('"shortName" option', () => {
      it('allows custom shortNames', async () => {
        ...
        const result = await agent.evalScript('$testing.evalScript("print(1)")');
    ...
    describe('"transform" option', async () => {
      let agent;
